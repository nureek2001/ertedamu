...
import { ActivityCategory, CATEGORY_META } from '../activities/data';
...
const MONTHLY_REMINDERS: Record<number, { title: string; items: string[] }> = {
  ...
  11: { title: '11 месяцев', items: ['Осмотр у педиатру', 'Проверка мелкой моторики и лепета'] },
...
const MCHAT_RECOMMENDATIONS = {
  low: {
    title: 'Всё в порядке',
    text: 'Ваш результат указывает на низкий риск. Продолжайте заниматься по программе развития.',
    color: '#166534',
    bg: '#DCFCE7',
  ...
  medium: {
    ...
    text: 'Средний риск. Рекомендуется обсудить результаты с вашим педиатром.',
    color: '#92400E',
    bg: '#FEF3C7',
    icon: 'alert-circle',
  },
  high: {
    ...
    text: 'Высокий риск требует обязательной консультации специалиста (невролога).',
    color: '#991B1B',
    bg: '#FEE2E2',
    icon: 'warning',
  },
...
type ScreeningSession = {
  ...
  score: number | null;
  result_level: 'low' | 'medium' | 'high' | 'done' | null;
  started_at: string;
  completed_at: string | null;
  target_age_months: number | null;
  answers?: ScreeningAnswer[];
  summary?: string | null;
  detailed_analysis?: ScreeningAnalysisItem[];
...
type ScreenStatus =
  | {
      type: 'mchat';
      status: 'low' | 'medium' | 'high' | 'done' | null;
      score: number | null;
      date: string;
      summary?: string | null;
      detailed_analysis?: ScreeningAnalysisItem[];
      general_recommendations?: string[];
      raw: ScreeningSession;
    }
  | {
      type: 'early';
      targetAge: number;
...
const MyChildScreen: React.FC = () => {
  ...
  const [mchatStatus, setMchatStatus] = useState<ScreenStatus>(null);
  const [isScreeningAvailable, setIsScreeningAvailable] = useState(true);
  const [activityCatalog, setActivityCatalog] = useState<BackendActivity[]>([]);
  const [milestoneProgressData, setMilestoneProgressData] = useState<MilestoneProgressResponse | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const childrenMapped: ChildChip[] = childrenRes.map((child) => ({
        id: String(child.id),
        name: child.first_name || 'Ребёнок',
      ...
      setChildrenList(childrenMapped);

      const activeBackendId = activeRes?.active_child?.id ? String(activeRes.active_child.id) : String(childrenRes[0].id);
      let resolvedIndex = childrenMapped.findIndex((c) => c.id === activeBackendId);
      ...
      const currentId = String(currentChild.id);

      setCurrentChildAge(currentChild.age_months || 1);

      const savedConfirmedRaw = await AsyncStorage.getItem(`confirmed_stages_${currentId}`);
      if (savedConfirmedRaw) {
        setConfirmedStages(JSON.parse(savedConfirmedRaw));
      } else {
        setConfirmedStages([]);
      }

      const [historyRes, milestonesRes, activitiesCatalogRes] = await Promise.all([
        ...
        api.get<BackendActivity[]>(`/api/activities/?child_id=${currentId}`),
      ]);

      setCompletedActivities(historyRes || []);
      setMilestoneProgressData(milestonesRes || null);
      setActivityCatalog(activitiesCatalogRes || []);
      ...
      try {
        ...
        console.log('LATEST SCREENING =', JSON.stringify(latestScreening, null, 2));
    } catch (e: any) {
      ...
      if (latestSameType) {
        const screeningDate = new Date(latestSameType.completed_at || latestSameType.started_at);
        const sameMonth =
          screeningDate.getMonth() === now.getMonth() &&
          screeningDate.getFullYear() === now.getFullYear();

        setIsScreeningAvailable(!sameMonth);
      } else {
        setIsScreeningAvailable(true);
      }
      ...
      if (latestScreening) {
        if (latestScreening.template?.code === 'mchat') {
          setMchatStatus({
            ...
            summary: latestScreening.summary || null,
            detailed_analysis: latestScreening.detailed_analysis || [],
            general_recommendations: latestScreening.general_recommendations || [],
            raw: latestScreening,
          });
  ...
  const renderEarlyResults = () => {
    if (!mchatStatus || mchatStatus.type !== 'early') return null;

    const targetAge = mchatStatus.targetAge;
    const analysis = Array.isArray(mchatStatus.detailed_analysis) ? mchatStatus.detailed_analysis : [];

    if (!analysis.length && !mchatStatus.summary) return null;
    ...
    return (
      <View style={styles.resultsCard}>
        ...
        {!!mchatStatus.summary && <Text style={styles.resultsSummary}>{mchatStatus.summary}</Text>}
        ...
        {analysis.map((item) => {
          ...
          return (
            <View key={item.question_id || item.order} style={styles.resRow}>
              <View style={styles.resTop}>
                <View style={[styles.resDot, { backgroundColor: color }]} />
                <Text style={styles.resQText}>{item.question_text}</Text>
              </View>

              {!!item.recommendation && (
                <View style={styles.resRecBox}>
                  <Text style={styles.resRecText}>💡 Рекомендация: {item.recommendation}</Text>
                </View>
              )}
            </View>
          );
        ...
        {!!mchatStatus.general_recommendations?.length && (
          <View style={styles.generalRecWrap}>
            {mchatStatus.general_recommendations.map((rec, idx) => (
              <View key={`${rec}-${idx}`} style={styles.generalRecRow}>
                <Ionicons name="checkmark-circle" size={14} color="#6366F1" />
                <Text style={styles.generalRecText}>{rec}</Text>
              </View>
            ))}
          </View>
        )}
  ...
  const renderMchatResults = () => {
    if (!mchatStatus || mchatStatus.type !== 'mchat') return null;
    ...
    return (
      <View style={styles.resultsCard}>
        <View style={styles.resultsHeader}>
          ...
          <Text style={styles.resultsTitle}>Анализ скрининга</Text>
        </View>
              ...
              <View key={item.question_id || item.order} style={styles.resRow}>
                ...
                <View style={styles.resRecBox}>
                  <Text
                    style={[
                      styles.resRecText,
                      { color: isRisk ? '#B91C1C' : '#166534' },
                    ]}
                  >
                    {isRisk ? 'Ответ попал в зону риска' : 'Ответ в пределах нормы'}
                  </Text>
                </View>
              </View>
            );
  ...
  const handleCompleteTest = async () => {
    ...
    if (testProgress.length === STAGE_TESTS[stageMonths].items.length) {
      ...
      await AsyncStorage.setItem(
        `confirmed_stages_${currentChild.id}`,
      ...
      <View style={styles.journeySection}>
        <View style={styles.sectionHeader}>
          ...
          <View style={styles.totalProgressBadge}>
            <Text style={styles.totalProgressText}>{Math.round((currentChildAge / 72) * 100)}% пути</Text>
          </View>
        </View>
        ...
        <ScrollView
          ...
          contentContainerStyle={[styles.journeyScroll, { paddingTop: 40 }]}
        >
          {stages.map((stage, index) => {
            const isReached = currentChildAge >= stage.m;
            const isConfirmed = confirmedStages.includes(stage.m);
            const isCurrent =
              isReached && (index === stages.length - 1 || currentChildAge < stages[index + 1].m);
              ...
              <View key={stage.m} style={styles.journeyNode}>
                {index !== stages.length - 1 && (
                  <View
                    style={[
                      styles.connector,
                      { backgroundColor: currentChildAge > stage.m ? stage.color : '#E2E8F0' },
                    ]}
                  />
                )}
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    setTestProgress(isConfirmed ? STAGE_TESTS[stage.m].items : []);
                    setIsTestVisible(true);
                  }}
                  style={[
                    styles.nodeCircle,
                    {
                      backgroundColor: isReached ? stage.color : '#FFF',
                      borderColor: isReached ? stage.color : '#E2E8F0',
                    },
                    isCurrent && styles.nodeCircleCurrent,
                    isConfirmed && styles.nodeCircleConfirmed,
                  ]}
                >
                  ...
                  {isCurrent && !isConfirmed && (
                    <View style={styles.currentIndicator}>
                      <Text style={styles.currentIndicatorText}>ВЫ ТУТ</Text>
                    </View>
                  )}
                </TouchableOpacity>

                <Text style={[styles.nodeLabel, isReached && { color: '#1E293B', fontWeight: '800' }]}>
                  {stage.label}
                </Text>
  ...
  const completedActivityBackendIds = new Set(
    completedActivities.map((item) => String(item.activity.id))
  );

  const activityStats = (Object.keys(CATEGORY_META) as ActivityCategory[]).map((cat) => {
    const items = activityCatalog.filter((a) => a.category?.slug === cat);
    const done = items.filter((a) => completedActivityBackendIds.has(String(a.id))).length;
    const target = items.length;

    return {
      key: cat,
      meta: CATEGORY_META[cat],
      percent: Math.min(Math.round((done / (target || 1)) * 100), 100),
    };
  });

  const totalActPercent = Math.min(
    Math.round(activityStats.reduce((s, i) => s + i.percent, 0) / (activityStats.length || 1)),
    100
  );

  const milestoneStats = (Object.keys(MILESTONE_META) as MilestoneCategory[]).map((cat) => {
    const items = (milestoneProgressData?.items || []).filter(
      (item) => item.milestone.category?.slug === cat
    );

    const done = items.filter((item) => !!item.progress?.is_completed).length;
    const target = items.length;

    return {
      key: cat,
      meta: MILESTONE_META[cat],
      done,
      target,
      percent: Math.min(Math.round((done / (target || 1)) * 100), 100),
    };
  ...
  const totalMilPercent = Math.min(
    Math.round(milestoneStats.reduce((s, i) => s + i.percent, 0) / (milestoneStats.length || 1)),
    100
  );
  ...
  return (
    <SafeAreaView style={styles.screen} edges={['top']}>
      ...
      >
        ...
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>Здоровье и тесты</Text>
        </View>
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            isScreeningAvailable
              ? router.push({
                  pathname: screeningInfo.path as any,
                  params: { childId: childrenList[activeChildIndex]?.id },
                })
        ...
        >
          ...
          >
            <Ionicons
              name={!isScreeningAvailable ? 'stats-chart' : (screeningInfo.icon as any)}
              size={24}
              color="#FFF"
            />
          </LinearGradient>
          ...
          <View style={styles.lockInfo}>
            {!isScreeningAvailable && <Ionicons name="lock-closed" size={16} color="#94A3B8" />}
        ...
        {!isScreeningAvailable &&
          mchatStatus &&
          mchatStatus.type === 'mchat' &&
          !mchatStatus.detailed_analysis?.length && (
            <View
              style={[
                styles.recommendationCard,
                {
                  backgroundColor:
                    MCHAT_RECOMMENDATIONS[
                      mchatStatus.status as keyof typeof MCHAT_RECOMMENDATIONS
                    ].bg,
                },
              ]}
            >
        ...
        {activeTab === 'activities' ? (
          <View style={styles.statsPage}>
            <LinearGradient colors={['#4F46E5', '#7C3AED']} style={styles.mainProgressCard}>
              <View style={styles.progressInfo}>
                <Text style={styles.progressValue}>{totalActPercent}%</Text>
                <Text style={styles.progressLabel}>Развитие навыков</Text>
              </View>
              ...
              {activityStats.map((item) => (
                <View key={item.key} style={styles.gridItem}>
                  <View
                    style={[
                      styles.itemIcon,
                      { backgroundColor: item.meta.gradient[0] + '20' },
                    ]}
                  >
                    <Text style={{ fontSize: 20 }}>{item.meta.emoji}</Text>
                  </View>
                  ...
                  <View style={styles.miniBar}>
                    <View
                      style={[
                        styles.miniBarFill,
                        { width: `${item.percent}%`, backgroundColor: item.meta.gradient[0] },
                      ]}
                    />
                  </View>
                </View>
          ...
          <View style={styles.statsPage}>
            <LinearGradient colors={['#10B981', '#059669']} style={styles.mainProgressCard}>
              <View style={styles.progressInfo}>
                <Text style={styles.progressValue}>{totalMilPercent}%</Text>
                <Text style={styles.progressLabel}>Освоено этапов</Text>
              </View>
            ...
            {milestoneStats.map((item) => (
              <View key={item.key} style={styles.milestoneRow}>
                <View style={[styles.milestoneIcon, { backgroundColor: item.meta.color }]}>
                  <Text style={{ fontSize: 18 }}>{item.meta.emoji}</Text>
                </View>
                  ...
                  <View style={styles.milestoneBar}>
                    <View
                      style={[
                        ...
                        { width: `${item.percent}%`, backgroundColor: item.meta.color },
                      ]}
                    />
                  </View>
                </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalBody}>
              ...
              {STAGE_TESTS[selectedStage?.m]?.items.map((item, idx) => (
                <TouchableOpacity key={idx} style={styles.checkItem} onPress={() => toggleTestItem(item)}>
                  <Ionicons
                    name={testProgress.includes(item) ? 'checkbox' : 'square-outline'}
                    size={24}
                    color={testProgress.includes(item) ? selectedStage?.color : '#94A3B8'}
                  />
                  <Text style={[styles.checkText, testProgress.includes(item) && styles.checkTextActive]}>
                    {item}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            <View style={styles.modalFooter}>
              <TouchableOpacity style={styles.btnCancel} onPress={() => setIsTestVisible(false)}>
                <Text style={styles.btnCancelText}>Закрыть</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.btnConfirm, { backgroundColor: selectedStage?.color }]}
                onPress={handleCompleteTest}
              >
                <Text style={styles.btnConfirmText}>
                  {confirmedStages.includes(selectedStage?.m) ? 'Понятно' : 'Подтвердить'}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
      ...
      <Modal visible={isHistoryVisible} animationType="slide" transparent={false}>
        ...
        >
          <View style={styles.historyHeaderEnhanced}>
            <TouchableOpacity onPress={() => setIsHistoryVisible(false)} style={styles.closeButtonCircle}>
            ...
            {Object.entries(MONTHLY_REMINDERS).map(([month, data]) => {
              ...
              return (
                <View key={month} style={styles.timelineNode}>
                  <View style={[styles.timelineDot, isPast && styles.dotPast, isCurrent && styles.dotCurrent]}>
                  ...
                  <View style={[styles.enhancedHistoryCard, isCurrent && styles.cardCurrentActive, isPast && styles.cardPast]}>
                    {isCurrent && (
                      <LinearGradient colors={['#6366F1', '#818CF8']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.currentBadgeGradient}>
...
const styles = StyleSheet.create({
  ...
  miniBar: { height: 4, backgroundColor: '#F1F5F9', borderRadius: 2, marginTop: 8 },
  ...
  milestoneBar: { height: 6, backgroundColor: '#F1F5F9', borderRadius: 3, marginTop: 6 },
  ...
  resultsSummary: { fontSize: 13, color: '#64748B', lineHeight: 19, marginBottom: 14, fontWeight: '500' },
  ...
  generalRecWrap: { marginTop: 6, paddingTop: 8, borderTopWidth: 1, borderTopColor: '#F1F5F9' },
  generalRecRow: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: 8 },
  generalRecText: { flex: 1, fontSize: 12, color: '#475569', lineHeight: 17, marginLeft: 8, fontWeight: '500' },
});